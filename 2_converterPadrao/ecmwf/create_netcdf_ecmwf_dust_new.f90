...
program create_netcdf_ecmwf_dust_new
    use date_utils
    use netcdf
    use netcdf_utils
    use ecmwf_dust_new
...
!        outputFile=trim(outputDir)//'/'//institutionCode//'_'//ccase//'_'//subcase//'_'//dateStrMinute//'.nc'
        ...
        call write3dVars(dateStrDay,institutionCode,ncid_out,vars3d,vars3dVectorSize)
...
!            call check(nf90_get_att(nc3did_in, varId_in,"missing_value", undef3d))
    ...
    subroutine write3dVars(dateStrDay,institutionCode,ncid_out,vars3d,vars3dVectorSize)
        implicit none
        character(len=8), intent(in) :: dateStrDay
        character(*), intent(in) :: institutionCode
        integer, intent(in) :: ncid_out
        ...
        print*, "ESCREVENDO VARIAVEIS 3D"
        do i = 1, vars3dVectorSize
		    input3dFile=getInput3dVarFileImpl(trim(vars3d(i)%nameIn),dateStrDay)	
            call openFile(nc3did_in, input3dFile)
            call check(nf90_inq_varid(nc3did_in,trim(vars3d(i)%nameIn), vars3d(i)%id))
			do iTime = 1, timeIn3d

				call check(nf90_get_var(nc3did_in, vars3d(i)%id, vars3d(i)%value, start =(/1,1,1,iTime/), &
                	count = (/ lonIn, latIn, levIn, 1 /) ))

	            call check(nf90_put_var(ncid_out, vars3d(i)%id, vars3d(i)%value, start =(/1,1,1,iTime/), &
                	count = (/ lonIn, latIn, levIn, 1 /) ))
			end do
...
end program create_netcdf_ecmwf_dust_new
