...
contains
    ...
    subroutine create3dVars(dateStrDay,institutionCode,latId,lonId,timeId,levId,ncid_out,vars3d,vars3dVectorSize)
	    ...
	    input3dFile=getInput3dVarFileImpl(trim(vars3d(i)%nameIn),dateStrDay)	
            ...
            call check(nf90_inq_varid(nc3did_in,vars3d(i)%nameIn, varId_in))
...
!            call check(nf90_get_att(nc3did_in, varId_in,"missing_value", undef3d))
            ...
            call check(nf90_def_var(ncid_out, vars3d(i)%nameIn, NF90_REAL, (/lonId, latId, levId, timeId/), vars3d(i)%id))
            call check(nf90_put_att(ncid_out, vars3d(i)%id, 'standard_name', vars3d(i)%nameIn))
            call check(nf90_put_att(ncid_out, vars3d(i)%id, 'long_name', vars3d(i)%longName))
            call check(nf90_put_att(ncid_out, vars3d(i)%id, 'units', vars3d(i)%units))
